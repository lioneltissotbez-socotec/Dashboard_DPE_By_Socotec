// DPE Explorer - logique applicative séparée
...
async function apiFetch(url, btnId, statusId) {
  ...
  async function tryFetchJson(fetchUrl, label) {
    console.log('[DPE Explorer] fetch ' + label, fetchUrl);
  ...
  try {
    try {
      const d = await tryFetchJson(url, 'proxy');
      clearStatus(statusId);
      return d;
    } catch(eProxy) {
      // Certains paramètres full-text/qs peuvent être refusés par le Worker Cloudflare.
      // En cas de 403, on tente l'URL ADEME directe pour distinguer problème proxy / problème requête.
      const canFallbackDirect = eProxy.status === 403 && url.startsWith(PROXY_BASE + '/?');
      if (!canFallbackDirect) throw eProxy;

      const directUrl = ADEME + '?' + url.split('/?')[1];
      console.warn('[DPE Explorer] Proxy 403. Tentative ADEME directe.', { proxyUrl: url, directUrl });
      const d = await tryFetchJson(directUrl, 'ADEME direct après 403 proxy');
      clearStatus(statusId);
      return d;
    }
  } catch(e) {
    console.error('[DPE Explorer] Erreur API', e);
    setStatus(statusId, 'err', '✖ ' + e.message + '<br><span style="font-size:11px;color:var(--muted)">Voir la console navigateur pour le détail. Si c\'est un 403 sur le proxy, le problème vient du Worker, pas forcément de l\'API ADEME.</span>');
    return null;
  } finally {
    btn.disabled = false;
    btn.textContent = btnId === 'btn-search' ? '→ Rechercher'
...
function dpeLatLon(d) {
  ...
  if (!gp || !String(gp).includes(',')) return null;
  const parts = String(gp).split(',');
...
async function fetchJsonSmart(params) {
  ...
  urls.push({ label:'ademe-direct', url: direct });
  if (proxied) urls.push({ label:'proxy', url: proxied });
  ...
  throw lastErr || new Error('Aucune réponse API');
}
...
async function searchAddrNearbyMap(num, rue, ville, size, sort) {
  ...
  const cp = extractCpFromInput(ville);
  ...
  setStatus('addr', 'info', '<span class="spin">↻</span> &nbsp;Géocodage de l\'adresse puis recherche des DPE proches...');
  console.groupCollapsed('[DPE Explorer] Mode carte proximité');
  console.log('Saisie utilisateur', { numero:num, rue, ville, radius });
  try {
    const center = await geocodeAddressBAN(fullAddress);
    console.log('Adresse BAN retenue', center);
    const effectiveCp = cp || center.postcode || '';
    const effectiveCity = center.city || cleanCityFromInput(ville);

    // Stratégie optimisée v3 :
    // 1) identifiant BAN exact si disponible : récupère directement les DPE à l'adresse BAN.
    // 2) nom_rue_ban + code_postal_ban en qs : récupère uniquement la rue dans le CP, donc beaucoup plus léger.
    // 3) adresse_ban/adresse_complete_brut en qs puis q texte en secours.
    // IMPORTANT : les tentatives qs étaient ignorées dans la v2 par un `if (!params.q) continue;`.
    const streetNormForQs = titleCaseStreetForAdeme(rue);
    const streetQsParts = [];
    if (streetNormForQs && effectiveCp) {
      streetQsParts.push('code_postal_ban:' + effectiveCp + ' AND nom_rue_ban:"' + escapeQsValue(streetNormForQs) + '"');
      streetQsParts.push('code_postal_ban:' + effectiveCp + ' AND adresse_ban:"' + escapeQsValue(streetNormForQs) + '"');
      streetQsParts.push('code_postal_ban:' + effectiveCp + ' AND adresse_complete_brut:"' + escapeQsValue(streetNormForQs) + '"');
    }

    const attempts = [
      center.banId ? { qs: 'identifiant_ban:' + center.banId, size:'200' } : null,
      ...streetQsParts.map(qs => ({ qs, size:'1000' })),
      { q: streetCpQuery, size:'1000' },
      { q: [rue, effectiveCity].filter(Boolean).join(' '), size:'1000' }
    ].filter(Boolean);
    ...
    for (const params of attempts) {
      ...
      try {
        document.getElementById('url-addr').innerHTML = 'GET &nbsp;<span>' + displayUrl(params) + '</span>';
        const data = await fetchJsonSmart(params);
        const rows = data.results || [];
        console.log('Candidats ADEME pour', params, rows.length, 'sur total', data.total);
        candidates = candidates.concat(rows);

        // Si la requête ciblée rue + CP donne déjà assez de candidats proches, inutile d'aller chercher plus large.
        const tempUnique = new Map();
        candidates.forEach(d => { const id = d.numero_dpe || d._id || JSON.stringify(d).slice(0,80); if (!tempUnique.has(id)) tempUnique.set(id, d); });
        const tempNear = [...tempUnique.values()].map(d => {
          const ll = dpeLatLon(d);
          if (!ll) return null;
          return Object.assign({}, d, { _distance_m: distanceMeters(center.lat, center.lon, ll.lat, ll.lon) });
        }).filter(Boolean).filter(d => d._distance_m <= radius);
        if (tempNear.length > 0 && params.qs) {
          console.log('Arrêt anticipé : candidats proches trouvés avec requête ciblée', tempNear.length);
          break;
        }
      } catch(e) { console.warn('Tentative ADEME échouée', params, e.message); }
    }
    const byId = new Map();
    candidates.forEach(d => { const id = d.numero_dpe || d._id || JSON.stringify(d).slice(0,80); if (!byId.has(id)) byId.set(id, d); });
    candidates = [...byId.values()];
    const near = candidates.map(d => {
      const ll = dpeLatLon(d);
      if (!ll) return null;
      return Object.assign({}, d, { _distance_m: distanceMeters(center.lat, center.lon, ll.lat, ll.lon) });
    }).filter(Boolean).filter(d => d._distance_m <= radius).filter(passesAdvancedFiltersLocal).sort((a,b) => a._distance_m - b._distance_m);
    console.log('Candidats uniques', candidates.length);
    console.log('DPE dans le rayon après filtres', near.length);
    console.table(near.map(d => Object.assign(compactDpeForLog(d), { distance_m: Math.round(d._distance_m), geopoint: d._geopoint })));
    clearStatus('addr');
    const renderData = { total: near.length, results: near.slice(0, parseInt(size || '20', 10)), _mapSearch:true, _mapCenter:center, _mapRadius:radius, _mapCandidates:candidates.length };
    renderDpeMap(near, center, radius);
    renderAddrResults(renderData, fullAddress);
  } catch(e) {
...
function renderDpeMap(results, center, radius) {
  ...
  if (info) info.textContent = results.length + ' DPE dans ' + radius + ' m · ' + center.label;
  if (!addrLeafletMap) {
  ...
  L.marker([center.lat, center.lon], { icon: targetIcon }).addTo(addrLeafletLayer).bindPopup('<strong>Adresse recherchée</strong><br>' + center.label);
  L.circle([center.lat, center.lon], { radius, color:'#22d3ee', weight:1, fillColor:'#22d3ee', fillOpacity:0.08 }).addTo(addrLeafletLayer);
  const groups = new Map();
  results.forEach(d => {
    const ll = dpeLatLon(d); if (!ll) return;
  ...
  groups.forEach(g => {
    const rows = g.rows.sort((a,b) => (a.date_etablissement_dpe < b.date_etablissement_dpe ? 1 : -1));
    const first = rows[0];
    const label = rows.length > 1 ? rows.length : (first.etiquette_dpe || '?');
    const color = rows.length > 1 ? '#22d3ee' : dpeMarkerColor(first.etiquette_dpe);
    const cls = rows.length > 1 ? 'dpe-marker multi' : 'dpe-marker';
    const icon = L.divIcon({ className:'', html:'<div class="' + cls + '" style="background:' + color + '">' + label + '</div>', iconSize:[34,34], iconAnchor:[17,17] });
    L.marker([g.lat, g.lon], { icon }).addTo(addrLeafletLayer).bindPopup(buildMapPopup(rows), { maxWidth:340 });
  });
  const bounds = L.latLngBounds([[center.lat, center.lon]]);
  results.forEach(d => { const ll = dpeLatLon(d); if (ll) bounds.extend([ll.lat, ll.lon]); });
...
function buildMapPopup(rows) {
  ...
  html += '<div style="font-weight:700;color:#22d3ee;margin-bottom:6px">' + rows.length + ' DPE à cette position</div>';
  rows.forEach(d => {
    ...
    html += '<strong>DPE ' + (d.etiquette_dpe || '?') + '</strong> · GES ' + (d.etiquette_ges || '?') + ' · ' + (d.date_etablissement_dpe || 'date ?') + '<br>';
    html += (d.surface_habitable_logement ? d.surface_habitable_logement + ' m² · ' : '') + (d.type_batiment || '') + '<br>';
    html += '<span style="color:#94a3b8">' + (d.adresse_ban || d.adresse_complete_brut || d.adresse_brut || '') + '</span><br>';
    html += '<span style="color:#94a3b8">Distance : ' + Math.round(d._distance_m || 0) + ' m · N° ' + (d.numero_dpe || '') + '</span>';
    html += '<button class="map-popup-btn" onclick="loadDpeFromAddrB64(\'' + b64 + '\')">🔍 Ouvrir la fiche complète</button>';
    html += '</div>';
  });
  html += '</div>';
  return html;
